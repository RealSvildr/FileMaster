    ...
    public partial class Form1 : Form {
        ...
        private Image ResizeImage(Image img, Img im) {
            if (cImageResize.Checked) {
                ...
                if (cImageFixed.Checked) {
                    width = Convert.ToInt32(tImageMaxWidth.Text);
                    height = Convert.ToInt32(tImageMaxHeight.Text);
                    resizeP = ((width / im.Width) + (height / im.Height)) / 2;
                } else {
                    if (string.IsNullOrEmpty(tImageResizePercentage.Text)) {
                        ...
                        resizeP = Convert.ToDecimal(tImageResizePercentage.Text) / 100;
                    }

                    width = (int)(im.Width * resizeP);
                    height = (int)(im.Height * resizeP);
                }
                ...
                img.Dispose(); //Clears before getting a new image
                return destBitmap;
            }
        ...
        private void ImageCheckboxResize_CheckedChanged(object sender, EventArgs e) {
            tImageResizePercentage.Enabled = cImageResize.Checked;
            tImageMaxResize.Enabled = cImageResize.Checked;
            tImageQuality.Enabled = cImageResize.Checked;
        ...
        private void ImageCheckboxCrop_CheckedChanged(object sender, EventArgs e) {
            //if (cImageCrop.Checked) {
            //    cImageConvert.Checked = false;
            //    cImageResize = false;
            //}

            tImageCropStartX.Enabled = cImageCrop.Checked;
            tImageCropStartY.Enabled = cImageCrop.Checked;
            tImageCropEndX.Enabled = cImageCrop.Checked;
            tImageCropEndY.Enabled = cImageCrop.Checked;
        }
            ...
            if (cFolderFileByName.Checked) {
                ...
                foreach (var dir2 in dir.GetDirectories()) {
                    listFolderRemove.ForEach(d => {
                        try { dir2.MoveTo(dir2.FullName.Replace(d, "")); } catch (Exception) { }
                    });
